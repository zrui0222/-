...
parser.add_argument('--batch-size', type=int, default=100)  # 5e-4
...
metric_summary.update({'test_loss': test_loss,
                       'test_rho': test_rho,
                       'test_nmse': test_nmse,
                       'epoch': 0})
...
if args.temp_balance_lr != 'None':
    ...
    # create optimizer
    optimizer = optim.SGD(all_params, momentum=0.9, weight_decay=args.weight_decay)
else:
...
for epoch in range(start_epoch, start_epoch + args.num_epochs):
    ...
    # train and test
    train_loss = train(epoch, net, args.num_epochs, train_loader, criterion, optimizer)
    print("\n| Train Epoch #%d\t\t\tLoss: %.4f" % (epoch, train_loss))
    test_loss, test_rho, test_nmse = test(epoch, net, test_loader, criterion)
    print("\n| Validation Epoch #%d\t\t\tLoss: %.4f rho: %.4f  nmse: %.4f" % (epoch, test_loss, test_rho, test_nmse))
